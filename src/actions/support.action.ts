...
export async function sendSupportMessageAction(
  ...
  try {
    ...
    log.info("support_send_complete", {
      messageId: id,
      userId: session.user.id,
    });
    return { success: true };
...
> {
  ...
  try {
    const rows = await db
      ...
      .where(
        or(
          eq(platformConfig.key, "support_whatsapp"),
          eq(platformConfig.key, "support_email"),
        ),
      );
