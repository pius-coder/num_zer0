...
export async function updateUserAccountStatusAction(
  ...
  log.info("admin_update_account_status", {
    userId: input.userId,
...
export async function manualCreditAdjustmentAction(
  ...
  log.info("admin_credit_adjust_started", {
    userId: input.userId,
    amount: input.amount,
  });
  ...
  try {
    ...
    log.info("admin_credit_adjust_complete", {
      userId: parsed.userId,
  ...
  try {
    ...
    const [messagesResult] = await db
      ...
      .where(
        and(
          ...
          eq(supportMessages.direction, "user_to_admin"),
        ),
      );
    const unreadMessages = messagesResult?.value ?? 0;
    ...
    return {
      ...
      data: {
        ...
        unresolvedFraud: Number(unresolvedFraud),
  ...
  try {
    ...
    await db
      ...
      .set({
        ...
        ...(parsed.floorCredits !== undefined && {
          floorCredits: parsed.floorCredits,
        }),
