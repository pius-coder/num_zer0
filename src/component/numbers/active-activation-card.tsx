...
export const ActiveActivationCard = memo(function ActiveActivationCard({
  ...
  const stateConf: StateConfig =
    STATE_CONFIG[activation.state as keyof typeof STATE_CONFIG] ??
    STATE_CONFIG.requested;
    ...
    <div className="rounded-xl border bg-card p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className={cn("text-xs font-medium", stateConf.color)}>
            {stateConf.label}
          </span>
        </div>
        <span className="text-xs text-muted-foreground">
          {activation.creditsCharged} cr
        </span>
      </div>
