...
export class StructuredLogger {
  ...
  child(contextOrConfig?: Record<string, unknown> | StructuredLoggerConfig): StructuredLogger {
    ...
    const isConfig =
      contextOrConfig && Object.keys(contextOrConfig).some((k) => configKeys.includes(k))
  ...
  private emit(
    ...
    if (Object.keys(this.context).length > 0) {
      ...
      if (ctx.requestId) {
        entry.requestId = String(ctx.requestId)
        delete ctx.requestId
      }
      if (ctx.traceId) {
        entry.traceId = String(ctx.traceId)
        delete ctx.traceId
      }
      if (ctx.method) {
        entry.method = String(ctx.method)
        delete ctx.method
      }
      if (ctx.path) {
        entry.path = String(ctx.path)
        delete ctx.path
      }
      if (ctx.url) {
        entry.url = String(ctx.url)
        delete ctx.url
      }
      if (ctx.userId) {
        entry.userId = String(ctx.userId)
        delete ctx.userId
      }
      if (ctx.sessionId) {
        entry.sessionId = String(ctx.sessionId)
        delete ctx.sessionId
      }
      if (ctx.locale) {
        entry.locale = String(ctx.locale)
        delete ctx.locale
      }
      if (ctx.route) {
        entry.route = String(ctx.route)
        delete ctx.route
      }
      if (ctx.statusCode) {
        entry.statusCode = Number(ctx.statusCode)
        delete ctx.statusCode
      }
      if (ctx.durationMs) {
        entry.durationMs = Number(ctx.durationMs)
        delete ctx.durationMs
      }
      if (ctx.stream) {
        entry.stream = ctx.stream as 'app' | 'error' | 'access' | 'audit'
        delete ctx.stream
      }
