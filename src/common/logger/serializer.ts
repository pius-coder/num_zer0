 ...
 */
export function getDefaultOptions(overrides: Partial<SerializerOptions> = {}): SerializerOptions {
  return {
    depth: 0,
    maxDepth: 5,
    maxArrayLength: 100,
 ...
 */
function isAppError(value: unknown): value is Error & {
  code: string
  statusCode: number
 ...
 */
export function serializeInline(value: unknown, options: SerializerOptions): string {
  ...
  if (Array.isArray(value)) {
    ...
    return c ? paint(`Array(${value.length})`, ANSI.dim) : `Array(${value.length})`
  }
  ...
  if (isPlainObject(value)) {
    ...
    return c ? paint(`Object(${keys.length})`, ANSI.dim) : `Object(${keys.length})`
  }
...
function serializeError(
  ...
  if (isAppError(error)) {
    ...
    const codeVal = c ? paint(error.code, ANSI.yellow) : error.code
    lines.push(`${indent}  ${tee} ${codeLabel} ${codeVal}`)

    const statusLabel = c ? paint('status:', ANSI.dim) : 'status:'
    const statusVal = c ? paint(String(error.statusCode), ANSI.yellow) : String(error.statusCode)
    lines.push(`${indent}  ${tee} ${statusLabel} ${statusVal}`)

    if (error.details && Object.keys(error.details).length > 0) {
      const detailsLabel = c ? paint('details:', ANSI.dim) : 'details:'
      ...
      detailKeys.forEach((key, i) => {
        ...
        const fieldValStr = c ? paint(fieldVal, ANSI.white) : fieldVal
        lines.push(`${indent}    ${connector} ${fieldLabel}: ${fieldValStr}`)
      })
    }
  }
...
export function serializeTree(
  ...
  if (typeof value === 'object' && value !== null) {
    if (visited.has(value)) {
      const str = c ? paint('[Circular]', ANSI.red, ANSI.dim) : '[Circular]'
      return `${indent}${str}`
    }
    visited.add(value)
  }
  ...
  if (isMap(value)) {
    const lines: string[] = []
    const header = c ? paint(`Map(${value.size})`, ANSI.magenta) : `Map(${value.size})`
    ...
    limited.forEach(([key, val], i) => {
      const isLast = i === limited.length - 1 && limited.length === entries.length
      const connector = isLast ? corner : tee
      ...
      if (!isPrimitive(val) && typeof val === 'object' && val !== null) {
        ...
        lines.push(`${indent}  ${connector} ${keyStr} => ${valStr}`)
      }
    })
  ...
  if (isSet(value)) {
    const lines: string[] = []
    const header = c ? paint(`Set(${value.size})`, ANSI.magenta) : `Set(${value.size})`
    lines.push(`${indent}${header}`)
    ...
    limited.forEach((val, i) => {
      const isLast = i === limited.length - 1 && limited.length === values.length
      const connector = isLast ? corner : tee
      ...
      if (!isPrimitive(val) && typeof val === 'object' && val !== null) {
        ...
        lines.push(`${indent}  ${connector} ${serializeInline(val, options)}`)
      }
    })
  ...
  if (ArrayBuffer.isView(value) && !(value instanceof DataView)) {
    ...
    const header = c
      ? paint(`${typedArray.constructor.name}(${typedArray.length})`, ANSI.cyan)
      : `${typedArray.constructor.name}(${typedArray.length})`

    if (typedArray.length <= 10) {
      const preview = Array.from(typedArray.slice(0, 10) as unknown as ArrayLike<number>).join(', ')
  ...
  if (Array.isArray(value)) {
    ...
    const header = c ? paint(`Array(${value.length})`, ANSI.dim) : `Array(${value.length})`
    lines.push(`${indent}${header}`)

    const limited = value.slice(0, options.maxArrayLength)

    limited.forEach((item, i) => {
      const isLast = i === limited.length - 1 && limited.length === value.length
      const connector = isLast ? corner : tee
      const indexLabel = c ? paint(`[${i}]`, ANSI.dim) : `[${i}]`

      if (!isPrimitive(item) && typeof item === 'object' && item !== null) {
        ...
        lines.push(`${indent}  ${connector} ${indexLabel} ${serializeInline(item, options)}`)
      }
    })
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    if (!isPlain) {
      const header = c ? paint(constructorName, ANSI.cyan) : constructorName
      lines.push(`${indent}${header}`)
    }
    ...
    keys.forEach((key, i) => {
      ...
      if (!isPrimitive(val) && typeof val === 'object' && val !== null) {
        ...
        lines.push(`${indent}  ${connector} ${keyLabel}: ${serializeInline(val, options)}`)
      }
    })
 ...
 */
export function serialize(value: unknown, options: Partial<SerializerOptions> = {}): string {
...
export function serializeForJSON(value: unknown): unknown {
  ...
  function transform(val: unknown): unknown {
    ...
    if (isMap(val))
      return {
        __type: 'Map',
        entries: Array.from(val.entries()).map(([k, v]) => [transform(k), transform(v)]),
      }
