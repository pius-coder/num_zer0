...
import {
  ...
  paint,
  createBadge,
  formatTimestamp as formatAnsiTimestamp,
  formatPrefix,
  stripAnsi,
  getVisibleLength,
  padAnsi,
 ...
 */
export function formatForProduction(entry: LogEntry, config: LoggerConfig): string {
    ...
    for (const item of entry.data) {
      if (isError(item)) {
        ...
        if (appErr.details && typeof appErr.details === 'object') {
          jsonEntry.error.details = appErr.details as Record<string, string[]>
        }
      } else {
 ...
 */
function formatContext(context: Record<string, unknown>, colorize: boolean): string {
  const pairs = Object.entries(context).map(([key, value]) => {
    const keyStr = colorize ? paint(key, ANSI.dim) : key
    const valStr = colorize ? paint(String(value), ANSI.cyan) : String(value)
