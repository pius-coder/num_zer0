...
export interface LogStats {
  ...
  channels: Record<
    string,
    { files: number; sizeBytes: number; entries?: number }
  >;
  dates: string[];
  files: LogFileInfo[];
}
...
export function queryLogs(query: LogQuery = {}): LogQueryResult {
  ...
  if (query.date) {
    ...
    targetDates = [...new Set(allDiscovered.map((f) => f.date))].filter(
      (d) => d >= fromDate && d <= toDate,
    );
  } else {
    // Default: last 7 days of available data, or today
    const availableDates = [...new Set(allDiscovered.map((f) => f.date))]
      .sort()
      .reverse();
  ...
  const filesToRead = allDiscovered.filter(
    (f) =>
      targetChannels.includes(f.channel) &&
      (targetDates === null || targetDates.includes(f.date)),
  );
...
export function getLogStats(logDir?: string): LogStats {
  ...
  for (const f of files) {
    let channel = channels[f.channel];
    if (!channel) {
      channel = { files: 0, sizeBytes: 0 };
      channels[f.channel] = channel;
    }
    channel.files++;
    channel.sizeBytes += f.sizeBytes;
  }
