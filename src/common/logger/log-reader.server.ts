...
export function discoverLogFiles(logDir?: string): LogFileInfo[] {
  ...
  return files.sort((a, b) => b.date.localeCompare(a.date) || a.channel.localeCompare(b.channel))
}
...
export function queryLogs(query: LogQuery = {}): LogQueryResult {
  ...
  const filesToRead = allDiscovered.filter(
    (f) =>
      targetChannels.includes(f.channel) && (targetDates === null || targetDates.includes(f.date))
  )
