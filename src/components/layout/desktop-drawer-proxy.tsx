...
function DesktopDetailsModal({ mvt, onClose }: { mvt: Record<string, unknown>; onClose: () => void }) {
  return (
      ...
      <div className='relative bg-[var(--surface)] backdrop-blur-xl border border-[var(--line)]/50 rounded-2xl w-full max-w-md p-6 space-y-4'>
        ...
        <div className='space-y-3'>
          <div className='flex justify-between'>
            ...
            <span className='font-figtree text-[var(--sea-ink)] text-[18px] font-medium tracking-[-0.04em] text-right'>{String(mvt.label ?? '')}</span>
          </div>
          <div className='flex justify-between'>
            <span className='font-figtree text-[var(--sea-ink-soft)] text-[15px] font-semibold uppercase tracking-wider'>Montant</span>
            <span className={`font-figtree text-[18px] font-medium tracking-[-0.04em] ${Number(mvt.credit) > 0 ? 'text-[#25D366]' : 'text-[var(--sea-ink)]'}`}>
              {Number(mvt.credit) > 0 ? '+' : '-'}${(Number(mvt.credit) || Number(mvt.debit)).toFixed(2)} USD
            </span>
          </div>
          <div className='flex justify-between'>
          ...
          <div className='flex justify-between'>
            ...
            <span className='font-figtree text-[var(--sea-ink)] text-[18px] font-medium tracking-[-0.04em]'>
              {new Date(String(mvt.date)).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
            </span>
          </div>
          <div className='flex justify-between'>
...
export function DesktopDrawerProxy() {
  ...
  if (activePanel === 'details' && isOpen) {
    return <DesktopDetailsModal mvt={panelProps.transaction as Record<string, unknown>} onClose={closePanel} />
  }
