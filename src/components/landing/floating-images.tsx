  ...
  return (
    <>
      {FLOATING_IMAGES.map((card, i) => (
        <div key={i} className={`absolute z-1 ${positions[i].bottom} ${positions[i].right || positions[i].left || ""}`}>
          <div className="bg-dark-900 border border-white/15 rounded-[16px] px-4 py-3 w-[280px] backdrop-blur">
            ...
            <div className="flex gap-[6px] mt-[6px]">
              {card.badges.map(b => (
                <span key={b} className="font-figtree font-medium text-[10px] text-white/50 bg-white/8 px-2 py-[2px] rounded-[20px] tracking-[0.02em]">
                  {b}
                </span>
              ))}
            </div>
