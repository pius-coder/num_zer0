...
import { usePackages, useCreatePurchase } from '@/components/purchases/hooks'
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export function RechargeDrawer({ open, onOpenChange, preselectedPackageId }: RechargeDrawerProps) {
  ...
  const handleConfirm = useCallback(
    async (packageId: string, method: PaymentMethod) => {
      ...
      if (data.payment) window.location.href = data.payment.link
    },
    [createPurchaseMutation, packages]
  )
