...
export const WalletBalanceCard = memo(function WalletBalanceCard({
  ...
  return (
    <div className='space-y-3'>
      ...
      <div className='p-1'>
        {showLoading ? (
          <p className='font-figtree text-[var(--sea-ink-soft)] text-[15px] font-semibold uppercase tracking-wider'>Chargement du solde...</p>
        ) : (
          <div className='space-y-3'>
              ...
              <div className='flex items-center gap-2'>
                {onSync && (
                  <button
                    ...
                    title="Synchroniser"
                  >
                    <RefreshCw className={`h-4 w-4 text-[var(--sea-ink-soft)] ${syncing ? 'animate-spin' : ''}`} />
                  </button>
                )}
                {onRecharge && (
                  <button
