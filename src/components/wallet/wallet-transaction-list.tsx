...
export const WalletTransactionItem = memo(function WalletTransactionItem({
  ...
  const statutLabel = statut === 'validee' ? 'Validée' : statut === 'annulee' ? 'Annulée' : 'En attente'
  ...
  return (
    ...
    >
      ...
      <span className={`font-figtree text-[15px] font-semibold uppercase tracking-wider text-right ${
        statut === 'validee' ? 'text-[#25D366]' : statut === 'annulee' ? 'text-red-500' : 'text-yellow-500'
      }`}>
        {statutLabel}
      </span>
    </div>
