...
export class CreditLedgerService extends BaseService {
  ...
  async holdCredits(params: {
    ...
    await this.transaction(async (tx) => {
      ...
      for (const lot of lots) {
        ...
        this.assert(!!lotInfo, 'lot_not_found', 'Consumed lot not found', { lotId: lot.lotId })
  ...
  async confirmHoldDebit(holdId: string) {
    ...
    return this.db.query.creditHold.findFirst({ where: eq(creditHold.id, holdId) })
  }
  ...
  async releaseHold(holdId: string) {
    ...
    await this.transaction(async (tx) => {
      await tx
        .update(creditLot)
        .set({ remainingAmount: sql`${creditLot.remainingAmount} + ${hold.amount}` })
