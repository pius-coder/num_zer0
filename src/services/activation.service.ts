...
export class ActivationService extends BaseService {
  ...
  async request(input: RequestActivationInput) {
    ...
    try {
      ...
      try {
        await this.db.delete(smsActivation).where(eq(smsActivation.id, activationId))
  ...
  /**
   * Check Grizzly V1 status to detect if activation was cancelled externally.
   * Returns status info and updates DB if STATUS_CANCEL is detected.
   */
  async checkCancellationStatus(
    activationId: string
  ): Promise<{ status: 'cancelled' } | { status: 'active'; remainingSeconds?: number }> {
    const existing = await this.db.query.smsActivation.findFirst({
      where: eq(smsActivation.id, activationId),
    })

    if (!existing || !existing.providerActivationId) {
      this.log.warn('check_cancellation_activation_not_found', { activationId })
      return { status: 'active' }
    }

    const grizzly = getGrizzlyClient()
    const providerActivationId = Number(existing.providerActivationId)

    try {
      const v1Status = await this.withRetry(
        () => grizzly.getStatusV1(providerActivationId),
        'checkCancellationStatus'
      )

      // Handle STATUS_CANCEL - activation was cancelled on Grizzly side
      if (v1Status === 'STATUS_CANCEL') {
        this.log.info('grizzly_status_cancel_detected', {
          activationId,
      ...
      if (
        v1Status === 'STATUS_OK' ||
        v1Status === 'STATUS_WAIT_CODE' ||
        v1Status === 'STATUS_WAIT_RESEND'
      ) {
        return { status: 'active' }
      }

      // Handle object with remainingSeconds
      if (typeof v1Status === 'object' && v1Status !== null && 'remainingSeconds' in v1Status) {
        return {
          status: 'active',
          remainingSeconds: (v1Status as { remainingSeconds: number }).remainingSeconds,
        }
      }

      // Unknown status - fail safe, return active
      this.log.warn('grizzly_unknown_v1_status', {
        activationId,
        v1Status,
      })
      return { status: 'active' }
    } catch (err) {
      this.log.warn('check_cancellation_failed', {
        activationId,
        providerActivationId,
        error: err instanceof Error ? err.message : String(err),
      })
      // Fail safe - return active on error
      return { status: 'active' }
    }
  }
