...
export function SwipeToDelete({
    ...
    const startHapticSent = useRef(false)
    ...
    const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        if (!startHapticSent.current) {
            vibrate(10)
            startHapticSent.current = true
            setTimeout(() => {
                startHapticSent.current = false
            }, 120)
        }
    ...
    const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        if (!thresholdHapticSent.current && next >= maxDelta * 0.7) {
