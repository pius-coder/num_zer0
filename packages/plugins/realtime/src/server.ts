/**
 * @aura/realtime — broadcast server (Bun + Hono)
 *
 *   POST /publish   server-to-server invalidation, signed HMAC
 *   GET  /ws        WebSocket — clients receive INVALIDATE
 *   GET  /clients   active connection list (observability)
 *   GET  /health    liveness probe
 *
 * Dev   : bun packages/plugins/realtime/src/server.ts
 * Prod  : dedicated Bun container
 */
...
const PORT = Number(process.env.AURA_REALTIME_PORT ?? 3001);
const APP_ORIGIN = process.env.AURA_APP_URL ?? "http://localhost:3000";
const SECRET =
...
console.log(`[aura:realtime] listening on :${PORT}`);
