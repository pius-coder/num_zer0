...
Crée des tâches concrètes et claires. Les priorités reflètent l'urgence et l'importance.`)
  ...
  .handler(async ({ ctx, input }) => {
    const agentRef = { _name: "ai.todo-planner", __auraAgent: true as const };
    const thread = await ctx.agent.createThread(agentRef, { userId: ctx.user?.id });
    return ctx.agent.generateText(thread, { prompt: (input as { prompt?: string }).prompt ?? "" });
  });
